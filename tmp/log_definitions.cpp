 ...
 */
void load_xml(int index, std::string const & log_definitions_filename, watchdog_log_t::vector_t * result)
{
    snapdev::NOT_USED(index);

    QFile input(QString::fromUtf8(log_definitions_filename.c_str()));
    if(input.open(QIODevice::ReadOnly))
    {
        ...
        if(doc.setContent(&input, false)) // TODO: add error handling for debug
        {
            ...
            for(int idx(0); idx < max; ++idx)
            {
                ...
                if(name.isEmpty())
                {
                    throw log_definitions_exception_invalid_parameter(
                    ...
                    if(max_size_node.isElement())
                    {
                        ...
                        for(int j(0); j < size; ++j)
                        {
                            ...
                            else
                            {
                                ...
                                default:
                                    throw log_definitions_exception_invalid_parameter(
                                              "invalid size character \""
                                            + std::string(1, c)
                                            + "\" defining the size, should be one of K, M, G, k, m, or g.");
                                ...
                                if(j < size)
                                {
                                    ...
                                    {
                                        throw log_definitions_exception_invalid_parameter(
                                                  "left over characters after the size definition are not allowed");
                                    }
                                }
                                break;
                            }
                            ...
                            if(c >= '0' && c <= '8')
                            {
                                ...
                                && c == '/')
                                {
                                    for(++j; j < size; ++j)
                                    {
                                        ...
                                        {
                                            throw log_definitions_exception_invalid_parameter(
                                                      "a numeric mode must have a numeric mask");
                                        }
                            ...
                            else
                            {
                                ...
                                for(;;)
                                {
                                    ...
                                    case '=':
                                        ...
                                        {
                                            throw log_definitions_exception_invalid_parameter(
                                                      "only one operator can be used in a mode");
                                        }
                                    ...
                                    default:
                                        throw log_definitions_exception_invalid_parameter(
                                                  "unknown character for mode, expected one or more of u, g, o, or a");
                                ...
                                for(; j < size; ++j)
                                {
                                    ...
                                    default:
                                        throw log_definitions_exception_invalid_parameter(
                                                  "unknown character for mode, expected one or more of r, w, x, s, or t");
...
watchdog_log_t::vector_t watchdog_log_t::load(QString log_definitions_path)
{
    ...
    log_filenames.read_path<
        ...
        , snapdev::glob_to_list_flag_t::GLOB_FLAG_EMPTY>(std::string((log_definitions_path + "/*.xml").toUtf8().data()));
    snapdev::enumerate(
          log_filenames
        , std::bind(&load_xml, std::placeholders::_1, std::placeholders::_2, &result));
